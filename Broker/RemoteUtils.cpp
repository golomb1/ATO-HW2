...
void ToLowerCase(__in LPCSTR lpModuleName, __out PCHAR lpModuleNameCopy, __in DWORD size) {
	for (size_t i = 0; lpModuleName[i] != '\0' && i < size; ++i)
	{
...
HMODULE WINAPI GetRemoteModuleHandle(HANDLE hProcess, LPCSTR lpModuleName)
{
	...
	ToLowerCase(lpModuleName, lpModuleNameCopy, MAX_PATH);
